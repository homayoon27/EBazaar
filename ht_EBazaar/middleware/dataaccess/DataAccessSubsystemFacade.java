...
public class DataAccessSubsystemFacade implements IDataAccessSubsystem,
	...
	private static final Logger LOG = Logger
			.getLogger(DataAccessSubsystemFacade.class.getPackage().getName());
	DbAction action;
	Connection con;

	/**
	 * Gets a connection from the SimpleConnectionPool and caches it. Also
	 * caches the DbAction instance that is used. Note that all methods in
	 * DataAccess require createConnection as the first step. In
	 * saveInTransaction, createConnection is called explicitly. The save and
	 * delete methods have as a precondition that createConnection has already
	 * been called.
	 */
	...
	public void startTransaction() throws DatabaseException {
		try {
			...
			throw new DatabaseException(
					"DataAccessSubsystemFacade.startTransaction() "
	...
	public void rollback() throws DatabaseException {
		try {
			...
			throw new DatabaseException("rollback encountered a SQLException "
					+ e.getMessage());
		}
	}
	 ...
	 * must manually releaseConnection after read has completed. Can use
	 * atomicRead to handle createConnection and releaseConnection if no other
	 * data access code is bundled with the read.
	 */
	...
	/**
	 * Precondition: A Connection has been obtained via createConnection. User
	 * of this code must manually releaseConnection after operation has
	 * completed. For updates, returns number of rows affected; for inserts,
	 ...
	 * Precondition: A Connection has been obtained via createConnection. User
	 * of this code must manually releaseConnection after operation has
	 * completed. Returns number of rows deleted
	 */
	public Integer delete() throws DatabaseException {
		return action.performUpdate();
	}

	// //// Convenience methods for data access operations that are already
	// atomic ////////
	 ...
	 * use startTransaction instead. Note that createConnection is called as
	 * part of the method body (so a separate call to createConnection is not
	 * required in this case). Likewise, releaseConnection is handled
	 * automatically.
	 */
	...
	/**
	 * This convenience method carries out a typical delete within a
	 * transaction. To wrap multiple or complex sql operations in a transaction,
	 * use startTransaction instead. Note that createConnection is called as
	 * part of the method body (so a separate call to createConnection is not
	 * required in this case). Likewise, releaseConnection is handled
	...
	public ResultSet[] multipleInstanceQueries(String[] queries, String[] dburls)
		...
		SimpleConnectionPool pool = SimpleConnectionPool
				.getInstance(numConnections);
