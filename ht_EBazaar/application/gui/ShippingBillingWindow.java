...
/**
 * Class Description: This class presents a screen that displays shipping and
 * billing fields. The screen provides a Select Shipping Address button that
 * permits the user to pick, from another screen, one of the customer's
 * addresses, which is then loaded automatically into the shipping fields on the
 * current screen.
 */
public class ShippingBillingWindow extends JInternalFrame implements
		ParentWindow {
	...
	public boolean isNewShipAddress() {
		return (isNewShipAddrCheckbox != null ? isNewShipAddrCheckbox
				.isSelected() : false);
	}

	public boolean billingSameAsShipping() {
		return (addressesSameCheckbox != null ? addressesSameCheckbox
				.isSelected() : false);
	}

	/**
	 * Used by ShipAddressesWindow and by CheckoutController to fill in the
	 * shipping address
	 */
	public void setShippingAddress(String name, String addr, String city,
	...
	public void setBillingAddress(String name, String addr, String city,
			String state, String zip) {
	...
	private void defineLower() {
		...
		selectShipButton.addActionListener(control
				.getSelectShipButtonListener(this));

		// proceed checkout button
		JButton proceedCheckoutButton = new JButton(PROCEED_WITH_CHECKOUT);
		proceedCheckoutButton.addActionListener(control
				.getProceedFromBillingCheckoutListener(this));
		...
		// create lower panel
		JButton[] buttons = { selectShipButton, proceedCheckoutButton,
				backButton };
